  ...
  const getLocationDetails = async (id) => {
    try {
      ...
      const response = await axios.get(
        `${url}/location-management/locations/${id}/`,
        {
          headers: {
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogTrigger asChild>
          <Button
            ...
            onClick={() => onHandleLocationEdit(locationId)}
          >
            <span>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-4">
                    <p className="flex items-center gap-2">
                      ...
                      Location Admin Name :{" "}
                      {locationDetails?.location_admin?.name}
                    </p>
                    <p className="flex items-center gap-2">
                      <span>
                        <FaMobileRetro size={20} />
                      </span>
                      Mobile : {locationDetails?.location_admin?.mobile}
                    </p>
                    <p className="flex items-center gap-2">
                      <span>
                        <MdEmail size={20} />
                      </span>
                      Email : {locationDetails?.location_admin?.email}
                    </p>
                    <p className="flex items-center gap-2">
                      <span>
                        <FaPersonRays size={20} />
                      </span>
                      Role : {locationDetails?.location_admin?.role}
                    </p>
                  </div>
                </div>
                <Separator />
                <div className="flex items-center justify-end gap-2">
                  <Common_Dialog_Create_and_Update
                    heading="Update"
                    description="This form will allow you to update location for the hotels."
                    action="Update"
                    location={locationDetails}
                  />
                  <Button
                    className="bg-red-600 hover:bg-red-500"
                    onClick={() => onHandleLocationDelete(locationId)}
                  >
                    Delete
                  </Button>
                </div>
